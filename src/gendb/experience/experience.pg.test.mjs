#!/usr/bin/env node
/**
 * Experience Graph — PostgreSQL backend tests.
 *
 * Validates the same three access patterns as experience.test.mjs, but on the
 * Postgres/pgvector backend (graph traverse via recursive CTE, relation join,
 * and vector search via the HNSW ANN index).
 *
 ...
 *
 *   GENDB_EXPERIENCE_PG_TEST=postgres://user@host/db node src/gendb/experience/experience.pg.test.mjs
 */
...
async function main() {
  ...
  await store.close();
  console.log(`\n✅ experience.pg.test.mjs — ${passed} assertions passed`);
}
