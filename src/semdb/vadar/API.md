...
patch.best_ocr_match(options) -> str
    ...
    # PREFER over classify for WORDMARK logos / large value spaces (printed names) where
...
```python
# SMALL value space (a handful of names) -> classify picks the best of them
def extract(patch):
    return {"venue": patch.classify(VENUE_VALUES, "the logo of {}")}

# LARGE value space of names PRINTED in the image -> OCR reads the wordmark
def extract(patch):
    return {"brand": patch.best_ocr_match(BRAND_VALUES)}   # 'none' self-gates non-logos

# A category enum plus a colour attribute -> classify (CLIP) + dominant_colors (CV)
def extract(patch):
    return {"product_type": patch.classify(CATEGORY_VALUES),
...
- SMALL/visual value space or a category enum -> `classify`. LARGE value space of legible
  WORDMARK names (brands with legible text) -> `best_ocr_match`. Colors -> `dominant_colors`.
